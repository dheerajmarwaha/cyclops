...
//https://roytuts.com/html-table-conditional-cell-color-using-react/
  ...
  getColor(value) {
    if (value >= 0 && value <= 15) {
      ...
      return "RGBA(105,170,90,0.7)";
        ...
        return "red";//"RGBA(115,164,76,0.6)";
    } else if (value > 30000) {
          ...
          <tbody>
            {this.state.products.map(
              function (product, key) {
                return (
                  <tr key={key}>
                    <td style={{ backgroundColor: this.getColor(product.id) }}>
                      {product.id}
                    </td>
                    <td
                      style={{ backgroundColor: this.getColor(product.price) }}
                    >
                      {product.price}
                    </td>
                    <td
                      style={{
                        backgroundColor: this.getColor(product.sale_price),
                      }}
                    >
                      {product.sale_price}
                    </td>
                    <td
                      style={{
                        backgroundColor: this.getColor(product.sales_count),
                      }}
                    >
                      {product.sales_count}
                    </td>
